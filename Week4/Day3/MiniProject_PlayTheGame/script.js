...
function playTheGame() {
  function repeatAnswer() {
    do {
      userRequest = Number(prompt("Enter a number between 0 and 10"));
    } while (!(userRequest >= 0 && userRequest <= 10));
  }

  var answer = confirm("Would like to play the game?");
  if (!answer) {
    ...
    if (userRequest < 0 && userRequest > 10) {
      alert(`Sorry it’s not a good number, Goodbye`);
      repeatAnswer();
    } else {
      var computerNumber = Math.trunc(Math.random() * 10) + 1;
    }
    console.log(computerNumber);

    compareNumbers(userRequest, computerNumber);

    // return userRequest, computerNumber;
  }
}
...
function compareNumbers(userNumber, computerNumber) {
  let newNumber = 0;
  function loopingHere() {
    for (let i = 0; i < 2; i++) {
      newNumber = Number(prompt("Enter a new number"));
      if (newNumber === computerNumber) {
        alert("WINNER");
        break;
      }
      if (newNumber < computerNumber) {
