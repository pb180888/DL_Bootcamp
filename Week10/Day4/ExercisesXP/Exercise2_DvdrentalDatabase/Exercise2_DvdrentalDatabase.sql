...
-- select film_id, title, description, length, rental_rate from film where title IN ('Iron Man');
...
-- select customer.first_name, customer.last_name, payment.amount, payment.payment_date from customer inner join payment on payment.customer_id = customer.customer_id order by payment.customer_id asc;
...
-- ON film.film_id = inventory.film_id where inventory_id is NULL; 

-- 14. Write a query to find which city is in which country.
-- select city.city || ' is from '|| country.country from city inner join country 
...
-- select customer.customer_id, customer.first_name || ' ' || customer.last_name as full_name, payment.amount, payment.payment_date, payment.staff_id from payment 
-- inner JOIN customer on payment.customer_id = customer.customer_id order by staff_id asc;
